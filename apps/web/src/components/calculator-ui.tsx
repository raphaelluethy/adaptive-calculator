...
export function CalculatorUI() {
    ...
    const isSDUIEnabled = featureFlagsData.find((f) => f.flag === "sdui")?.value;
                ...
                <div className="grid grid-cols-4 gap-2 pt-2">
                    {uiData.buttons.map((button) => (
                        <Button
                            ...
                            className={`${themes[selectedTheme].button} ${button.className === "operator"
                            ...
                            disabled={
                                (button.value === "." &&
                                    !featureFlagsData.find((f) => f.flag === "decimal-numbers")
                                        ?.value) ||
                                (button.value === "+/-" &&
                                    !featureFlagsData.find((f) => f.flag === "negative-numbers")
                                        ?.value)
                            }
                        >
                            {button.label}
